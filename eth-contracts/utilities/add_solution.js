...
const proof = require("../../zokrates/code/square/proofs/9/proof.json");
...
(async() => {
  ...
  try {
    let result = await contract.methods.addSolution(proof.proof.a, proof.proof.b, proof.proof.c, proof.inputs, tokenId).send({ from: accounts[0], gas: 2500000});
    console.log(result)
  } catch(err) {
    throw (err);
  }
